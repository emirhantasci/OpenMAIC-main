...
export interface GenerationToolbarProps {
  language: 'zh-CN' | 'en-US' | 'tr-TR';
  onLanguageChange: (lang: 'zh-CN' | 'en-US' | 'tr-TR') => void;
  webSearch: boolean;
    ...
    <div className="flex items-center gap-1 flex-wrap">
      ...
      <Popover>
          ...
          <TooltipTrigger asChild>
            <PopoverTrigger asChild>
              <button className={pillMuted}>
                ...
                <span>{language === 'zh-CN' ? '中文' : language === 'tr-TR' ? 'TR' : 'EN'}</span>
              </button>
            </PopoverTrigger>
          </TooltipTrigger>
        ...
        <PopoverContent align="start" className="w-auto p-1">
          {(['zh-CN', 'en-US', 'tr-TR'] as const).map((lang) => (
            <button
              key={lang}
              onClick={() => onLanguageChange(lang)}
              className={cn(
                'w-full px-3 py-1.5 text-left text-xs rounded hover:bg-muted transition-colors flex items-center gap-2',
                language === lang && 'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300',
              )}
            >
              {lang === 'zh-CN' ? '中文' : lang === 'tr-TR' ? 'Türkçe' : 'English'}
              {language === lang && <Check className="size-3 ml-auto" />}
            </button>
          ))}
        </PopoverContent>
      </Popover>
